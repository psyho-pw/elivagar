...
export class PoTokenAdapter implements IPoTokenService, OnModuleInit {
  ...
  async refreshPoToken(): Promise<PoTokenData> {
    if (this.isRefreshing) {
      this.loggerService.verbose('refreshPoToken', 'Token refresh already in progress');
    ...
    try {
      ...
      this.loggerService.verbose('refreshPoToken', 'VisitorData fetched successfully');
      ...
      this.loggerService.info(
        'refreshPoToken',
        `poToken generated successfully (length: ${poToken.length})`,
      );

      return this.tokenData;
    } catch (error) {
      this.loggerService.error('refreshPoToken', error, 'Error generating poToken');
  ...
  async handleCron(): Promise<void> {
    this.loggerService.info('handleCron', 'Scheduled poToken refresh started');
    await this.refreshPoToken();
  }
}
